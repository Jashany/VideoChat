...
export default function Controls({
  ...
  const ControlButton = ({ onClick, active, isActive, icon, label, danger = false, recording = false, disabled = false }) => (
    <div className="relative group">
      <button
        ...
        disabled={disabled}
        type="button"
        className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 flex items-center justify-center rounded-full transition-all duration-200 shadow-lg hover:shadow-xl cursor-pointer flex-shrink-0 ${
          disabled
      ...
      <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-2 sm:px-3 py-1 sm:py-1.5 bg-black/90 text-white text-[10px] sm:text-xs rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
        {label}
  ...
  return (
    <div className="flex flex-row items-center py-2 px-2 sm:px-4 relative z-20 overflow-x-auto">
      <div className="flex-1 flex justify-center items-center gap-2 sm:gap-3 md:gap-4 lg:gap-6 min-w-max mx-auto">
        <ControlButton
        ...
        <ControlButton
          ...
          icon={<ScreenShare className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />}
        />
        ...
        <ControlButton
          ...
          icon={
            isRecording ? (
              <Square className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
            ) : (
              <Circle className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
            )
          }
        />
        ...
        <ControlButton
          ...
          isActive={isChatOpen}
          label="Toggle chat"
          icon={<MessageCircle className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />}
        />
        ...
        <ControlButton
          ...
          icon={<Users className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />}
        />
        ...
        <ControlButton
          ...
          danger={true}
          label="Leave meeting"
          icon={<PhoneOff className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />}
        />
      </div>
    </div>
  );
